...
def install_and_register_pywin32():
    """Installs the pywin32 library and registers its components."""
    try:
        # Install pywin32 using pip
        subprocess.check_call([sys.executable, "-m", "pip", "install", "pywin32"])
        # It's crucial to register the libraries for usage with COM components
        from pywin32_bootstrap import ensure
        ensure()
        # Run post-install script to register components
        subprocess.check_call(["python", "Scripts/pywin32_postinstall.py", "-install"])
        print("pywin32 installed and registered successfully.")
    except subprocess.CalledProcessError as e:
        print(f"An error occurred during installation or registration: {e}")
    except ImportError as e:
        print(f"Failed to import necessary modules after installation: {e}")

if __name__ == "__main__":
    install_and_register_pywin32()
